...
- **节奏呼吸**：不许全章一个呼吸频率——情感峰值/兑现点/场景转换处放 1-2 处长句呼吸（约平常句的两倍长），与周围短句形成落差；每章长句（≥基线上限×2 字）至少 5 个（写后 style_baseline 会检测，过扁报 advisory）
...
| `context` 上下文整份提交 | 上章 `long_term_constraints`/`continuity_risks` 两清单：已了结的项**逐字**填 `delta.retired_context_items`，未了结的原样带回——掉落未申报直接拒收 |
...
3. **闸门对照**（修订后必做）：逐点核对 spec「在场与时间闸门」的四条答案与正文——每个「几点几分」、每处在场/离场、每件物件状态是否与正文一致；**凡改动过计数/数量/时间表述，重数一遍**（修订引入计数矛盾是最高频回归）
4. 收手原则：只改病句不改剧情风格（一个 pass 只干一件事）
