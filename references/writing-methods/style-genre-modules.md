...
正文写作采用「通用正文要求 + 题材正文提示卡 + 本书文风」三件套，不再为每个题材复制一整套正文 prompt；冲突时：本章蓝图与连续性 > 题材卡 > 本书文风 > 通用建议。三件套分工、召回规则、题材卡写法与番茄校准红线（不强制行长/对话占比、不全局替换、不随机倒装等）详见 `../genre-prose-cards.md` 索引与 `genre-prose-cards/{题材}.md` 单卡；本文件只作无匹配卡时的通用流派补充——此时从下方各题材模块抽主题材 3-5 条、辅题材 1-2 条写成轻量 `genre_prose_card`，每章只带本章相关条目，不得改剧情顺序、不得替换角色人设、不得覆盖 `outline.md` 章纲与 `tracking/context.md` 连续性状态的权威召回。
