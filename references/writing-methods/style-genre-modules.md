...
- **通用正文要求**由主线 novel-draft 子技能负责：严格消费本章蓝图 `spec.md`，按情节点预算写，缓慢推进，不提前写后续剧情，完成字数/钩子/禁用词/退化校验。
...
- **本书文风**来自 `.novel/style-anchor.md`（作者级）或对标书 `benchmark/{书名}/文风.md`：只管句长、标点、潜台词、锚点片段和笔调，不覆盖题材核心与章节意图。
- 三者冲突时：本章蓝图（`spec.md`）与连续性 > 题材正文提示卡 > 本书文风 > 通用技巧建议。

### 题材正文提示卡模板

项目可在开书时生成题材提示卡（放书目录或 `references/`）。没有该文件时，写前先用 `concept.md` 匹配 `../genre-prose-cards.md` 索引并读取 `genre-prose-cards/{题材}.md` 单卡；仍无匹配再从本文件即时抽取一张轻量卡。卡片保持短，写正文时只传本章相关摘要，不把参考文件整段复制进 prompt。
...
1. 先从 `concept.md` 读取主题材、目标平台、核心梗（主对标书如有登记也读）。
...
5. 题材卡不得改剧情顺序、不得替换角色人设、不得覆盖 `outline.md` 章纲与 `tracking/context.md` 连续性状态的权威召回。
