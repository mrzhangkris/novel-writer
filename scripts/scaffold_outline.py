...
篇幅解析：concept.md 的「目标篇幅：」或「篇幅：」行，支持「约 3 万字」「15 万字起」「8 万字」「2 万字内」
（万=10000；未命中时按平台×类型的总字数档推默认章数，再不行默认 30 章，提示手动 --chapters）。
"""
...
def parse_total_words(root: Path) -> int | None:
    ...
    for line in concept.read_text(encoding="utf-8").splitlines():
        # book_init 蓝本写「- 篇幅：」，旧格式写「目标篇幅：」——两种标签都认
        m = re.search(r"(?:目标篇幅|篇幅)[：:]\s*(.+)$", line)
        if not m:
            continue
        text = m.group(1)
...
def total_range(root: Path) -> tuple[int, int] | None:
    """平台×类型的总字数档（篇幅解析失败时用它推默认章数）。"""
    pipe = read_json(root / PIPELINE_FILE)
    if not isinstance(pipe, dict):
        return None
    wc_file = Path(__file__).resolve().parent.parent / "references" / "word-count.json"
    ...
    if not plat:
        return None
    cfg = plat.get(pipe.get("type") or "长篇小说")
    rng = cfg.get("总字数") if isinstance(cfg, dict) else None
    return (int(rng[0]), int(rng[1])) if rng and len(rng) == 2 else None
...
def scaffold(chapters: int) -> str:
    ...
    lines += [
        ...
        "",
        "## 情绪曲线",
        "- 待定（逐章写「第N章【低/中/高】：情绪要点」——【】里的强度会被 pacing.md 直接采用；再兼顾小爽/中爽/大爽分布）",
        "",
        "## 伏笔规划",
        "",
        "| 编号 | 内容 | 埋设章 | 计划回收章 | 类型 |",
...
def main() -> int:
    ...
    if chapters is None:
        ...
        if total and rng:
            ...
            # 篇幅没写/没解析出来：按平台×类型的总字数档中值推，而不是拍脑袋 30 章
            trng = total_range(root)
            if trng:
                mid_total = (trng[0] + trng[1]) / 2
                chapters = max(1, math.ceil(mid_total / ((rng[0] + rng[1]) / 2)))
                print(f"⚠️  未解析到目标篇幅，按平台×类型总字数档 {trng[0]}~{trng[1]} 推默认 {chapters} 章（可用 --chapters 指定）")
            else:
