...
name: novel-init
description: |
  小说初始化：扫描目录 → 建作者工作区骨架 → 对话式定方向（10 问）→ 书级建目录。novel-writer 流水线第 1 步。
...
**首次 vs 之后**：preferences.md 无「默认选题」→ 完整 10 问，定完把可复用项存为默认；已有默认 → 先问「沿用吗」，沿用则只问「这次讲什么新故事」。
...
9. **文风**：本书用什么腔调写？（A 按 author-dna 作家 DNA——金庸招式拆解/古龙冷峻短句/辰东天地异象等 9 选 1；B 自述参考作家或作品；C 不确定——按题材默认：都市高武→古龙+蝴蝶蓝混合，女频古言→丁香情绪腔）。**选定后必须回填 `.novel/style-anchor.md`**（腔调关键词 3 词 + 样板段落，量化基线留第 3 章后校准）——空锚 = 每章文风漂移无约束
10. **故事名**：作者给 / AI 提议 3-5 个 / 暂定

**对标拆书**（10 问后默认建议走，作者明确跳过才省略）：问有没有对标书 → 走 `skills/branch/story-long-analyze/SKILL.md`，产出 `deconstruct/{书名}/`（文风样本留在 `deconstruct/{书名}/文风.md`，与 `.novel/style-anchor.md` 不混写）；爽点/节奏结论写进 concept「对标与差异化」。
