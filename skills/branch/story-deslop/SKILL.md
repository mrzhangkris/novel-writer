...
name: story-deslop
version: 1.0.0
description: "网文去AI味。检测并清除文本中的AI写作痕迹，让文字回归自然、非模板化。触发：/去AI味、「去AI味」「这篇太AI了」「网文去AI味」。"
...
**检测层升级（2026-08 调研吸收 avoid-ai-writing-zh / reduce-ai-novel-taste / humanize）**：
check-ai-patterns.js 在原有 20 类之外新增 5 类硬表层检测——AI 官僚词（赋能/底层逻辑/颗粒度等 Tier1 单个即报、Tier2 聚集报）、网文陈词与直白情绪标签（倒吸一口凉气/眼底闪过一丝等）、报幕式过渡（就在这时/紧接着等聚集报）、句长均匀度（连续 4 句长度差<5 且无短句=AI 节奏指纹）、第一人称「我们」密度漂移。全部正则/统计实现，零 token。语义层（perplexity/GLTR/剧情模板/人物扁平）不可脚本化，仍归冷读裁判。

**检测层升级 II（2026-08-15 融合 unslop gap 分析）**：再增 4 类零 token 检测——段落起手词重复（同一连接/时间词 ≥3 段开头，advisory）、散文式收口（综上所述/总而言之等总结腔起手，4-8 段短文形状时 blocking，否则 advisory）、零宽字符 U+200B/200C/200D/FEFF（blocking，隐写/反检测痕迹）、项目级扩展禁用词（`--extra-words <file>` 文件，聚集 ≥2 处 advisory）。

**去AI味三件套**（改写后自检）：
1. 语域一致：对白像人话、叙述不混入会议腔；
2. AI 味指数定位：先跑 check-ai-patterns.js 拿命中位置，再定点改，别整章盲改；
3. 虚词密度校准：改写后把「了/的/地/就/都」密度与 style-anchor 基线比对，偏离即回查（仿人工配比，防越改越机械）。

**职责边界**：本支线只改「AI 味」（句式/书面腔/节奏工整/修饰词）；机械病句（语序/成分残缺/搭配/指代/标点歧义）走 story-polish 支线，别混改。

**核心信念：AI 味的主要问题并非语法错误；更常见的是过度圆滑、工整、解释充分。改写目标是保留剧情功能，同时增加口语、停顿、跳跃和具体动作。**

---

> DSH 适配：无 agent 目录 / `/story-setup` 机制。需要独立上下文时用 subagent 工具 spawn 子代理（prompt 自带全部规则，见 Phase 3「执行方式」），否则主线程 inline 执行。
...
### 例外：auto-clarity 直白保留（unslop 吸收）

安全说明、法规条文、医疗描述等必须字面准确的内容，不做口语化/风格化改写，保留原样并在报告标 `[直白保留]`；去AI味只作用于叙事段落。
...
```bash
node {SKILL_DIR}/skills/branch/story-deslop/scripts/check-ai-patterns.js --check --fail-on=blocking <正文文件...>
```
...
> 量化标准为参考值。命中 = banned-words.md 中条目作为连续字符串在文本中出现一次。同一词在一处出现计 1 次。
>
...
#### 执行方式：inline 或子代理（去AI味执行）

「诊断与分级」完成后，按以下顺序选择执行路径：

1. **文件模式默认 spawn 子代理**：全文重写是重上下文工作，主线程只做摘要与采纳；子代理按协议读正文执行 Gate A-G，避免污染主会话上下文。短片段（一段以内）才 inline。
2. **需要独立上下文时**：用 subagent 工具 spawn 一个「去AI味执行」子代理，prompt 必须包含：项目目录、待处理的正文文件路径、AI味等级（轻/中/重）、处理策略（对应 Gate 范围）、删除优先原则、`references/anti-ai-writing.md` 与 `references/banned-words.md` 的加载路径。子代理只改写指定文件，不得动其他项目文件。
3. 无论 inline 还是子代理执行，均须遵守下文「删除优先判断」与各 Gate 规则。
...
项目根目录下的 `.deslop-whitelist` 文件定义本项目的豁免词汇（UTF-8，一行一个词；`#` 开头为注释，空行忽略）。检测时白名单词会被替换为等长占位符再匹配，命中片段含白名单词即跳过该次计数，适合世界观术语/绰号/设定专有名词。

```
node {SKILL_DIR}/skills/branch/story-deslop/scripts/check-ai-patterns.js --check --whitelist .deslop-whitelist chapters/chapter-NNN/draft.md
```

不传 --whitelist 时无豁免（主线 checks.py deai 闸门保持最严口径）；deslop 支线手动复扫时才带。

**项目级扩展禁用词**（unslop anti-aiisms.local.md 式）：项目根可放一个词表文件（UTF-8，一行一词，`#` 开头为注释，空行忽略），把本项目特有的 AI 腔词/作者禁用词加进去，聚集 ≥2 处即报 advisory：

```
node {SKILL_DIR}/skills/branch/story-deslop/scripts/check-ai-patterns.js --check --extra-words .deslop-extra-words chapters/chapter-NNN/draft.md
```

与 `.deslop-whitelist` 互补：whitelist 豁免计数，extra-words 新增计数。主线 checks.py deai 闸门不传 `--extra-words`（保持最严口径不变），deslop 支线手动复扫时才带。
...
```bash
node {SKILL_DIR}/skills/branch/story-deslop/scripts/check-ai-patterns.js --check --fail-on=blocking <正文文件...>
node {SKILL_DIR}/skills/branch/story-deslop/scripts/check-degeneration.js --check <正文文件...>
node {SKILL_DIR}/skills/branch/story-deslop/scripts/normalize-punctuation.js <正文文件...>
```
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 继续写作 | novel-writer 主流程 | 用 novel-writer 主流程（novel-draft）继续 |
| 发现结构问题 | story-long-analyze | 触发 story-long-analyze（拆书对标） |
...
## 五档强度（2026-08-15 融合 unslop 5 档 + humanize-chinese 风格变换）

| 档 | 适用 | 做法 |
|---|---|---|
| 1 轻修 | 单点 AI 味（个别词/句） | 定点改词，不动结构 |
| 2 中修 | 段落级（3-5 处） | Pass 1+2，句序可微调 |
| 3 重修 | 全文多处（>15 处/千字或 4+ Gate 命中） | 完整 7 Gate + 重点段落重写 |
| 4 重写 | 段落功能保留的全句重写 | 保留剧情功能/伏笔/钩子，句句重写 |
| 5 风格变换 | 整体腔调不对（文风锚要求冷峻却写成甜宠） | 按 style-anchor 换叙事腔调重写（白描化/口语化/文言化） |

**档间判别**（同一句按三档对比，看改动幅度定档）：
- 原文：「他感到很愤怒，于是用力拍了一下桌子。」（直陈情绪 + 「于是」）
- 1 轻修：「他拍了一下桌子。」
- 3 重修：「桌子上的水杯跳了起来。」（动作展示，句序重排）
- 5 风格变换：「他喉结滚了滚，一掌下去，桌上的灰都震了起来。」（按 style-anchor 换腔调）

只改词 = 1，动句序 = 3，换腔调 = 5；4 重写 = 句句重写但保留剧情功能。
