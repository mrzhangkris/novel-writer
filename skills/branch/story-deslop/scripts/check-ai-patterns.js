...
const USAGE = `Usage: node check-ai-patterns.js [--check] [--json] [--fail-on=blocking|all] [--whitelist <file>] [--extra-words <file>] <file...>
...
// ── 2026-08 调研吸收：AI 官僚词/网文陈词/过渡短语/句长均匀度/第一人称密度 ──
// 来源：avoid-ai-writing-zh、reduce-ai-novel-taste、humanize/ai-check、stop-slop。

// Tier1：单个命中即告警（网文正文出现即违和）
const AI_BUREAUCRAT_TIER1 = [
  '赋能', '底层逻辑', '颗粒度', '闭环', '抓手', '心智', '对齐', '拉通',
  '拆解', '梳理', '说白了', '本质上', '换言之', '总而言之', '综上所述',
];
// Tier2：聚集（同章 ≥3 处）才告警
const AI_BUREAUCRAT_TIER2 = [
  '值得注意', '不难发现', '由此可见', '毫无疑问', '显而易见', '毋庸置疑',
  '与此同时', '除此之外', '不仅如此', '更重要的是', '关键在于',
];

// 网文陈词滥调 + 直白情绪标签（advisory，网文可少量使用但密集即机械）
const NOVEL_CLICHES = [
  '倒吸一口凉气', '嘴角勾起一抹', '眼底闪过一丝', '深邃的眼眸', '眸中闪过一丝',
  '脸色一变', '心中一惊', '心里咯噔一下', '眉头一皱', '嘴角抽搐',
  '眸光一沉', '眼神一凛', '大手一挥', '冷笑一声', '不屑地撇了撇嘴',
  '语重心长地说', '意味深长地看了', '若有所思', '感慨万千',
];

// 报幕式/路标式过渡（聚集告警）
const TRANSITION_MARKERS = [
  '就在这时', '突然之间', '果不其然', '紧接着', '随后', '随即',
  '不一会儿', '转眼间', '说时迟那时快',
];
...
const PARA_OPENER_SUSPECTS = ['与此同时', '就在这时', '紧接着', '一时间', '另一边', '然而', '然后', '于是', '可是', '但是', '不过', '随后', '接着', '随即', '突然', '终于', '此刻', '此时', '只见', '却见'];
const PARA_OPENER_MIN_REPEAT = 3;

// 散文式收口（unslop essay-shape 吸收）：总结腔起手段 + 五段式短文形状 = 散文，不是小说。
const ESSAY_SUMMARY_OPENERS = ['综上所述', '总而言之', '一言以蔽之', '归根结底', '简而言之', '总的来看', '总之'];
const ESSAY_SHAPE_MIN_PARAS = 4;
const ESSAY_SHAPE_MAX_PARAS = 8;

// 零宽字符（unslop punctuation tells 吸收）：正文出现不可见字符即异常（隐写/反检测痕迹）。
const ZERO_WIDTH_CHAR_PATTERN = /[\u200B\u200C\u200D\uFEFF]/g;

// 项目级扩展禁用词（unslop anti-aiisms.local.md 吸收）：--extra-words 文件每行一词，# 注释。
let EXTRA_TERMS = [];
...
for (let i = 2; i < process.argv.length; i += 1) {
  ...
  if (arg === '--check') {
    ...
    if (eq > 0) options.whitelist = arg.slice(eq + 1);
    else {
      i += 1;
      options.whitelist = process.argv[i];
    }
    if (options.whitelist === undefined) die('--whitelist requires a file path');
  } else if (arg.startsWith('--extra-words')) {
    const eq = arg.indexOf('=');
    if (eq > 0) options.extraWords = arg.slice(eq + 1);
    else {
      i += 1;
      options.extraWords = process.argv[i];
    }
    if (options.extraWords === undefined) die('--extra-words requires a file path');
...
if (options.extraWords) {
  EXTRA_TERMS = loadExtraWords(options.extraWords);
}
...
for (const file of options.files) {
  ...
  if (options.whitelist) {
    input = applyWhitelist(input, loadWhitelist(options.whitelist));
  }
...
function loadWhitelist(whitelistPath) {
  ...
  try {
    ...
    die(`--whitelist file unreadable: ${error.message}`);
  }
  return terms;
}

// 白名单词替换为等长占位符「□」（非空白，保持可见长度统计近似不变），
// 使命中片段含白名单词时被拆断、跳过计数；词表本身不参与匹配。
function applyWhitelist(input, terms) {
...
// 项目级扩展禁用词文件（anti-aiisms.local.md 式）：每行一词，# 注释，空行忽略。
function loadExtraWords(extraWordsPath) {
  const terms = [];
  try {
    const raw = fs.readFileSync(extraWordsPath, 'utf8');
    for (const line of raw.split(/\r?\n/)) {
      const s = line.trim();
      if (!s || s.startsWith('#')) continue;
      terms.push(s);
    ...
    die(`--extra-words file unreadable: ${error.message}`);
  }
  return terms;
}


function findWordlistTics(proseLines) {
  const findings = [];
  const tier1Hits = [];
  const tier2Hits = [];
  const clicheHits = [];
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    for (const w of AI_BUREAUCRAT_TIER1) {
      if (narrative.includes(w)) tier1Hits.push({ lineNo, w });
    }
    for (const w of AI_BUREAUCRAT_TIER2) {
      if (narrative.includes(w)) tier2Hits.push({ lineNo, w });
    }
    for (const w of NOVEL_CLICHES) {
      if (narrative.includes(w)) clicheHits.push({ lineNo, w });
    }
    for (const w of TRANSITION_MARKERS) {
      if (narrative.includes(w)) transitionHits.push({ lineNo, w });
    }
  }

  for (const { lineNo, w } of tier1Hits) {
    findings.push({
      line: lineNo, column: 1, type: 'bureaucrat-word-tic', severity: 'advisory',
      message: `AI 官僚词「${w}」：网文叙事/对白里换成人话（对白可保留，讽刺语境人工判断）。`,
      excerpt: w,
    });
  }
  if (tier2Hits.length >= 3) {
    findings.push({
      line: tier2Hits[0].lineNo, column: 1, type: 'bureaucrat-word-tic', severity: 'advisory',
      message: `AI 路标词聚集 ${tier2Hits.length} 处（${tier2Hits.map((h) => h.w).join('/')}）：删掉或改成动作衔接。`,
      excerpt: tier2Hits.map((h) => h.w).join('、'),
    });
  }
  if (clicheHits.length >= 2) {
    findings.push({
      line: clicheHits[0].lineNo, column: 1, type: 'novel-cliche-tic', severity: 'advisory',
      message: `网文陈词 ${clicheHits.length} 处（${[...new Set(clicheHits.map((h) => h.w))].slice(0, 4).join('/')}）：换成角色专属的具体反应，别用通用表情模板。`,
      excerpt: clicheHits[0].w,
    });
  }
  if (transitionHits.length >= 3) {
    findings.push({
      line: transitionHits[0].lineNo, column: 1, type: 'transition-marker-tic', severity: 'advisory',
      message: `报幕式过渡 ${transitionHits.length} 处（${[...new Set(transitionHits.map((h) => h.w))].slice(0, 4).join('/')}）：用具体动作/时间锚点替代。`,
      excerpt: transitionHits[0].w,
    });
  }
  return findings;
}

// 句长均匀度：连续 ≥4 个叙述句长度差 <5 字、且整段没有 <8 字短句 → AI 节奏指纹
function findSentenceUniformityTic(proseLines) {
  const findings = [];
  let run = [];
  const flush = () => {
    if (run.length < 4) { run = []; return; }
    const lens = run.map((s) => s.len);
    const span = Math.max(...lens) - Math.min(...lens);
    const hasShort = lens.some((l) => l < 8);
    if (span < 5 && !hasShort) {
      findings.push({
        line: run[0].lineNo, column: 1, type: 'sentence-uniformity-tic', severity: 'advisory',
        message: `连续 ${run.length} 句长度几乎相同（${lens.join('/')} 字）：AI 节奏指纹。长短句交错，关键处砸短句。`,
        excerpt: compact(run[0].text),
      });
    }
    run = [];
  };
  for (const { text, lineNo } of proseLines) {
    ...
    const narrative = stripQuoted(trimmed);
    // 按句末标点拆句，只统计非对话叙述句
    const parts = narrative.split(/[。！？]/).filter((s) => s.trim());
    for (const part of parts) {
      const len = visibleLength(part);
      if (len === 0) continue;
      run.push({ len, lineNo, text: part });
      if (run.length > 6) run.shift();
      if (run.length >= 4) {
        const lens = run.map((s) => s.len);
        const span = Math.max(...lens) - Math.min(...lens);
        const hasShort = lens.some((l) => l < 8);
        if (span < 5 && !hasShort && run.length >= 4) {
          // 留待 flush 统一上报：此处即时判定一次
          flush();
        }
      }
    }
...
// 第一人称「我们」密度：第三人称网文叙述中出现即 AI 腔
function findWeDensityTic(proseLines) {
  let hits = 0;
  let chars = 0;
  let firstLine = null;
  for (const { text, lineNo } of proseLines) {
    ...
    if (!trimmed || isDivider(trimmed) || isStructural(trimmed)) continue;
    const narrative = stripQuoted(trimmed);
    chars += visibleLength(narrative);
    const count = (narrative.match(/我们/g) || []).length;
    if (count > 0 && firstLine === null) firstLine = lineNo;
    hits += count;
  }
  if (chars === 0 || hits === 0) return [];
  const perKilo = (hits / chars) * 1000;
  if (perKilo < 6) return [];
  return [{
    line: firstLine, column: 1, type: 'we-density-tic', severity: 'advisory',
    message: `叙述人称漂移：「我们」${hits} 处（${perKilo.toFixed(1)}/千字）。第三人称叙述里换具体的人名/人群，别用「我们」笼统带过。`,
    excerpt: '我们',
  }];
}


// 段落起手词重复（unslop para-opener 吸收）：同一连接/时间词在 ≥3 段开头重复。
function findParaOpenerRepeat(proseLines) {
  ...
  for (const { text, lineNo } of proseLines) {
    ...
    for (const word of PARA_OPENER_SUSPECTS) {
      if (narrative.startsWith(word)) {
        if (!counters.has(word)) counters.set(word, { count: 0, firstLine: lineNo });
        counters.get(word).count += 1;
        break;
      }
    }
  }
  const findings = [];
  for (const [word, { count, firstLine }] of counters) {
    if (count < PARA_OPENER_MIN_REPEAT) continue;
    findings.push({
      line: firstLine, column: 1, type: 'para-opener-repeat', severity: 'advisory',
      message: `段落起手词「${word}」重复 ${count} 次：连续段落别用同一个连接/时间词开头，换动作、人物或视角起段。`,
      excerpt: word,
    });
  }
  return findings;
...
// 散文式收口（unslop essay-shape 吸收）：总结腔起手 + 短文五段式形状 = 散文，不是小说。
function findEssayShape(proseLines) {
  const paras = proseLines.filter(({ text }) => {
    const t = text.trim();
    return t && !isDivider(t) && !isStructural(t);
  });
  for (const { text, lineNo } of paras) {
    const narrative = stripQuoted(text.trim()).trim();
    for (const opener of ESSAY_SUMMARY_OPENERS) {
      if (!narrative.startsWith(opener)) continue;
      const fiveParaShape = paras.length >= ESSAY_SHAPE_MIN_PARAS && paras.length <= ESSAY_SHAPE_MAX_PARAS;
      return [{
        line: lineNo, column: 1, type: 'essay-shape-zh',
        severity: fiveParaShape ? 'blocking' : 'advisory',
        message: fiveParaShape
          ? `散文式收口：「${opener}」起手的总结段 + 全文 ${paras.length} 段五段式形状。小说删总结段，用动作/场景收尾。`
          : `总结腔起手「${opener}」：小说正文删总结段，用动作/场景收尾。`,
        excerpt: opener,
      }];
    }
  }
...
// 零宽字符（unslop punctuation tells 吸收）：正文出现不可见字符即异常（隐写/反检测痕迹）。
function findZeroWidthChars(proseLines) {
  const findings = [];
  for (const { text, lineNo } of proseLines) {
    ZERO_WIDTH_CHAR_PATTERN.lastIndex = 0;
    let m;
    while ((m = ZERO_WIDTH_CHAR_PATTERN.exec(text)) !== null) {
      const code = m[0].codePointAt(0).toString(16).toUpperCase();
      findings.push({
        line: lineNo, column: m.index + 1, type: 'zero-width-char', severity: 'blocking',
        message: `零宽字符 U+${code}：正文出现不可见字符即异常（隐写/反检测痕迹），删除该字符。`,
        excerpt: `U+${code}`,
      });
    }
  }
...
// 项目级扩展禁用词（unslop anti-aiisms.local.md 吸收）：--extra-words 文件里的词，聚集 ≥2 处报。
function findExtraWordTics(proseLines) {
  if (EXTRA_TERMS.length === 0) return [];
  const hits = [];
  for (const { text, lineNo } of proseLines) {
    const trimmed = text.trim();
    if (!trimmed || isDivider(trimmed) || isStructural(trimmed)) continue;
    ...
    for (const word of EXTRA_TERMS) {
      if (narrative.includes(word)) hits.push({ lineNo, word });
    }
  }
  if (hits.length < 2) return [];
  const uniq = [...new Set(hits.map((h) => h.word))];
  return [{
    line: hits[0].lineNo, column: 1, type: 'local-extra-word', severity: 'advisory',
    message: `项目级扩展禁用词 ${hits.length} 处（${uniq.slice(0, 5).join('/')}）：按项目 --extra-words 规则处理。`,
    excerpt: hits[0].word,
  }];
}
...
function scanDocument(input) {
  // 剥掉文件头 UTF-8 BOM（编码标记，不是隐写字符；extract_chapters.py 同款 utf-8-sig 惯例）。
  input = input.replace(/^\uFEFF/, '');
  ...
  findings.push(...findWordlistTics(proseLines));
  findings.push(...findSentenceUniformityTic(proseLines));
  findings.push(...findWeDensityTic(proseLines));
...
function scanProsePatterns(proseLines) {
  ...
  findings.push(...findParaOpenerRepeat(proseLines));
  findings.push(...findEssayShape(proseLines));
  findings.push(...findZeroWidthChars(proseLines));
  findings.push(...findExtraWordTics(proseLines));
  return findings;
}
