...
Stage 1 停靠点交付物，输出 `deconstruct/{书名}/快速预览.md`。**字段全部取自 Stage 0/1 已产出数据**（概要.md + 黄金三章深度拆解），不引入需 Stage 2-6 才能算出的字段。与终态 `拆文报告.md` 向上兼容——「基本信息」「黄金三章评分」两节同名同结构，全量拆完后 `拆文报告.md` 是 `快速预览.md` 的超集。
...
示例：残玉问道 | 东方仙侠 | 4 | 3400 | 番茄（一行一段，五段竖线分隔，无则写「无」）
...
| 桥段标签 | 从桥段词表选命中项，逗号分隔；可补表外确有桥段；无匹配留空，不硬凑（词表见 deconstruction-notes.md） |
...
|---|---|---|---|---|
| {优越感/安全感/被理解/替代性复仇/关系补偿/认知惊喜等} | {故事如何持续交付} | {第N章/Px} | {下一层期待如何被立起} | {可保留情绪逻辑；必须替换具体settings/人物/事件} |
...
`settings/世界观/力量体系.md`：
- 文本描述，含等级、晋升条件。长篇注意多层级力量体系
- 内容 < 200 字且不构成独立体系 → 合并到 `settings/世界观/背景设定.md`，本文件可省略

`settings/世界观/地理.md`：
- 分布、主要区域、关键地点（文本描述）
- 内容 < 200 字 → 合并到 `背景设定.md`，本文件可省略

`settings/世界观/背景设定.md`：
- 核心规则 + 特殊设定
...
`settings/势力/{势力名}.md`（每个核心势力一个文件）：
- 名称 / 类型（门派/组织/家族/国家）/ 核心人物 / 立场倾向 / 与其他势力关系 / 关键事件
- 内容 >= 200 字时独立；不足合并到 `settings/世界观/背景设定.md`（不丢失信息）
...
输出 `deconstruct/{书名}/文风.md`。整书级写作技法视图，供 novel-writer 主流程 日更循环喂给 正文写手（novel-draft）。
...
| 不可模仿 | benchmark书缺陷或不适合本项目的技法 | 否 | <100 字 |
