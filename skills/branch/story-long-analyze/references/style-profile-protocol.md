...
`deconstruct/{书名}/文风.md` 是整书级写作技法视图，聚合：
...
- **写**：`deconstruct/{书名}/文风.md`（analyze 独占）
- **读**：该书被显式选为另一项目的外部benchmark时，由 novel-writer 主流程 首次引用同步到 `{项目}/benchmark/{书名}/文风.md`；novel-writer 主流程 读项目benchmark视图（回退deconstruct）。正在导入重建的本书（导入功能暂未内置）不走这条同步。
...
- 生成时间：{date}
- （可选）适用 benchmark 书路径：deconstruct/{书名}/
...
- 喜剧↔重击的转场手法：{benchmark书在 轻松→悲伤 锐角转场时用了什么手法，举 1-2 个原文锚点。}
...
> 每片段 300-500 字，**用于 正文写手（novel-draft） 写作时的范例片段**。从 `原文/原文.txt` 按章节分隔符切片。模仿手法、不抄字句。
...
> 优先覆盖项目可能用到、且benchmark书中样本充足的基调：紧张、悲伤/压抑、轻松/温馨、热血。按拆文里实际分布挑 4-6 段；缺哪个基调就写“本书该基调样本不足，跳过”，不要编造。

## 不可模仿
- {benchmark书的明显缺陷或不适合当前项目的技法。可选段落，可空。}
```
...
| 值 | 触发条件 | 下游处理 |
|---|---|---|
| `high` | 数据直接可读（如基调序列从摘要 grep 得出） | 正文写手（novel-draft） 优先采纳，覆盖默认 Gate |
| `med` | 从样本归纳且样本充足（如看章节基调走向、对话潜台词整理） | 正文写手（novel-draft） 参考，与默认 Gate 协商 |
| `low` | 样本不足/原文缺失 | 正文写手（novel-draft） 让位回默认 Gate（不强制采纳） |

## 可用性语义

- `文风可用：是` → 文风可用于写作，正文写手（novel-draft） 按 confidence 分级使用。
- `文风可用：否：{原因}` → 文风质量不足（例如原文缺失、锚点全是占位符）。查询方读取时返回「文风退化」标记，正文写手（novel-draft）跳过文风，按默认 Gates 写作，避免被误导。
...
精确决议表见 story-deslop 支线（Gate B 句式去套路 / Gate D 节奏 / Gate F 章末禁升华）。
