...
Stage 3 聚合后必须生成两个权威产物，供 Stage 5 报告摘要和 `novel-writer 主流程` 直接读取：
...
| 力量体系 | 名称、等级、晋升方式（文本描述） | `settings/世界观/力量体系.md`（>=200字独立，否则并入 背景设定） |
| 地理 | 分布、主要区域、关键地点（文本描述） | `settings/世界观/地理.md`（同上，>=200字独立） |
| 势力 | 门派/组织/家族/国家 | **每个势力一个文件** `settings/势力/{势力名}.md`（>=200字独立；不足合并到 `settings/世界观/背景设定.md`） |
| 核心规则 | 世界运转的基本规则 | `settings/世界观/背景设定.md` |
| 特殊设定 | 区别于现实的独特设定 | `settings/世界观/背景设定.md`（与核心规则同文件） |
| 金手指 | 见上方「金手指合并规则」 | `settings/世界观/金手指.md`（统一在 `settings/世界观/` 子目录下，不放扁平 `settings/金手指.md`） |
...
## 合成阶段事实保真（阶段 3-5：剧情/settings/角色/报告）
...
1. **硬事实必须可溯源**：力量等级、数值、距离、属性、势力数量、谁对谁说了什么、角色出场章节——这类硬事实写进settings/角色/报告前，回原文核对（摘要不足就直接读原文）；核到才写。
2. **缺失写「原文未明确」，禁编造填空**：原文没给的字段，写「原文未明确/节选未涉及」，绝不用近义项、别处的范例或惯例补一个看似合理的值。
3. **跨指代不串**：相邻实体的属性不互相挪用——坐骑等级≠骑手等级、A 的台词≠B 的台词、本章统计数字≠相邻章数字。

落盘后做一次**事实可溯源自检**：抽查settings/角色/报告里的硬事实，逐条能否 grep 回原文；命中即过，命中不到的改写为「原文未明确」或删除，不保留无源断言。此自检独立于下方三个比率——比率查归类质量，它查事实真伪。
...
1. **计算孤立比例**：孤立情节点数 / 总情节点数。若比例 < 5%，跳过后续归入步骤，把这些情节点写入散落情节.md 留档（与 output-templates 口径一致，保留钩子记录）
...
Stage 2 使用 章节提取子代理 并行处理（每章一个 agent，每批 5-8 个），不分块。
